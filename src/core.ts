...
export function getExposed(
  ...
  isUserFile: (sourceName: string) => boolean
      ...
      ...Array.from(findAll("ContractDefinition", ast), (c) => {
        ...
        return [
          ...
          spaceBetween(
            ...
            ...getFunctions(c, contractMap, isLibrary)
...
function isExternalizable(fnDef: FunctionDefinition): boolean {
  return (
    ...
    fnDef.implemented &&
    fnDef.parameters.parameters.every((p) => p.storageLocation !== "storage")
  );
}
...
function getType(
  ...
  location: StorageLocation = varDecl.storageLocation
): string {
  ...
  const type =
    ...
    (typeIdentifier.endsWith("_ptr") ? ` ${location}` : "");
  return type;
}
  ...
  for (const parent of parents) {
    for (const fn of findAll("FunctionDefinition", parent)) {
      if (fn.visibility === "internal" && !overriden.has(fn.id)) {
        res.push(fn);
      } else if (isLibrary && !overriden.has(fn.id)) {
        res.push(fn);
      }
